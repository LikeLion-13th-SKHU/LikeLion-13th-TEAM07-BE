...
import com.example.ie_um.community.api.dto.response.PostListResDto;
...
public class CommunityController {
    ...
    @GetMapping
    public RspTemplate<PostListResDto> getAllPosts() {
        List<PostResDto> resDtoList = communityService.getAllPosts();
        PostListResDto postListResDto = new PostListResDto(resDtoList);
        return new RspTemplate<>(
                HttpStatus.OK,
                "전체 게시글을 성공적으로 조회했습니다.",
                postListResDto);
    }
    ...
    @Operation(
            summary = "커뮤니티 글 좋아요 등록",
            description = "특정 게시글에 좋아요를 등록합니다."
    )
